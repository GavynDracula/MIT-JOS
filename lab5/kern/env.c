...
segment_alloc(struct Env *e, void *va, size_t len)
{
    ...
    for (i = 0; i < len; i += PGSIZE) {
        err = page_alloc(&page);
        if (err) 
            panic("segment_alloc: Allocate physical page failed. \n", err);
        err = page_insert(e->env_pgdir, page, va + i, PTE_U | PTE_W);
        if (err)
            panic("segment_alloc: Map physical page failed. %e\n", err);
    }
}
...
env_create(uint8_t *binary, size_t size)
{
    ...
    int error;
    struct Env *env;
    error = env_alloc(&env, 0);
    if (error < 0)
        panic("env_create: Allocate environment failed. %e", error);
